...
import { extractTrustedActorFromAuthorization } from './proxy.middleware';

// Conformance vectors. The same table is asserted against clinical-emr-service's
// and payment-service's actor.util.ts so the three hand-written verifiers
// cannot drift apart (backlog N-05/C-14).
...
describe('extractTrustedActorFromAuthorization', () => {
  it.each(accepted)('accepts %s', (_label, token) => {
    expect(extractTrustedActorFromAuthorization(`Bearer ${token}`, SECRET)).toEqual({
      accountId: 'account-1',
      role: undefined,
    });
  });

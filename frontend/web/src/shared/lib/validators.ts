...
const emailField = z
  .string()
  .trim()
  .max(FIELD_LIMITS.email, `Email cannot exceed ${FIELD_LIMITS.email} characters`)
...
const phoneField = z
  .string()
  .trim()
  .max(FIELD_LIMITS.phone, `Phone cannot exceed ${FIELD_LIMITS.phone} characters`)
...
export const registerFormSchema = z
  ...
  .refine(
    (d) => `${d.firstName} ${d.lastName}`.trim().length <= FIELD_LIMITS.fullName,
    {

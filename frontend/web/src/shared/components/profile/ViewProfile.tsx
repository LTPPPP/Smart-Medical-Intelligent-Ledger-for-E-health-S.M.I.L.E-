...
export function ViewProfile({ onEdit }: { onEdit: () => void }) {
    ...
    return (
        <div className="flex gap-6">
            {/* ── Left: Identity Card ─────────────────────────────── */}
            <div className="relative w-80 shrink-0 rounded-2xl border bg-card text-card-foreground shadow-sm dark:border-white/[0.08] dark:bg-transparent dark:shadow-[0_4px_24px_-1px_rgba(0,0,0,0.2)] dark:backdrop-blur-[10px]" style={typeof window !== "undefined" && document.documentElement.classList.contains("dark") ? { background: "rgba(255,255,255,0.03)" } : {}}>
                {/* Avatar */}
                <div className="flex flex-col items-center pt-6">
                    <div className="relative mb-4">
                        ...
                        <div className="relative flex h-24 w-24 items-center justify-center rounded-full border-2 dark:border-[rgba(146,205,253,0.3)] dark:shadow-[0_0_16px_rgba(146,205,253,0.25)]">
                            <div className="flex h-[84px] w-[84px] items-center justify-center rounded-full bg-muted dark:bg-[#272A2D]">
                                <User className="h-8 w-8 text-muted-foreground dark:text-[#C1C7CF]" />
                            </div>
                    ...
                    <div className="mt-1 flex items-center gap-2 text-muted-foreground dark:text-[#C1C7CF]">
                        <MapPin className="h-[10px] w-[10px] text-muted-foreground/60 dark:text-[#8B9199]" />
                        <span
                            className="text-base"
                            style={{ fontFamily: "var(--font-public-sans)" }}
                        >
                ...
                <div className="mx-6 mt-6 rounded-xl border px-3 py-[10px] dark:border-white/[0.05] dark:bg-[rgba(29,32,35,0.5)]">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                            <CheckCircle2 className="h-3 w-3 text-[#45F0CF]" />
                            <span
            ...
            {/* ── Right: Details Card ──────────────────────────────── */}
            <div className="flex-1 rounded-2xl border bg-card text-card-foreground shadow-sm dark:border-white/[0.08] dark:bg-transparent dark:shadow-[0_4px_24px_-1px_rgba(0,0,0,0.2)] dark:backdrop-blur-[10px]" style={typeof window !== "undefined" && document.documentElement.classList.contains("dark") ? { background: "rgba(255,255,255,0.03)" } : {}}>
                {/* Personal Info */}
                <div className="p-6">
                    <SectionHeading icon={<User className="h-3 w-3" />} label={t("profile.personalInfo", "Personal Information")} />
                ...
                <hr className="border-0 h-px mx-0" style={{ background: "linear-gradient(90deg,rgba(255,255,255,0) 0%,rgba(128,128,128,0.15) 50%,rgba(255,255,255,0) 100%)" }} />

                {/* Contact */}
                <div className="p-6">
                    <SectionHeading icon={<Shield className="h-3 w-3" />} label={t("profile.contact", "Contact")} />
                    <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-5">
                        <InfoField label={t("profile.email", "Email Address")}>
                            <div className="flex items-center gap-2">
                                ...
                                <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-[#45F0CF]" />
                            </div>
                        </InfoField>
                        ...
                        <div className="col-span-2">
                            <InfoField label={t("profile.address", "Primary Clinic Address")} value={p.address} />
                        </div>
                    </div>
                </div>

                <hr className="border-0 h-px" style={{ background: "linear-gradient(90deg,rgba(255,255,255,0) 0%,rgba(128,128,128,0.15) 50%,rgba(255,255,255,0) 100%)" }} />

                {/* Account */}
                <div className="p-6">
                    <SectionHeading icon={<Shield className="h-3.5 w-3.5" />} label={t("profile.account", "Account")} />
                    <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-5">
                        ...
                        <InfoField label={t("profile.accountType", "Account Type")} value={p.accountType} />
                        <InfoField label={t("profile.lastLogin", "Last Login")} value={p.lastLogin} />
                        <InfoField label={t("profile.memberSince", "Member Since")} value={p.memberSince} />
                    </div>
                </div>
                ...
                {/* Professional */}
                <div className="p-6">
                    <SectionHeading icon={<Stethoscope className="h-4 w-4" />} label={t("profile.professional", "Professional")} />
                    <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-5">
                        <InfoField label={t("profile.licenseNumber", "Medical License Number")}>
                            <div className="flex items-center gap-3">
                                <span
                                    className="text-sm text-foreground dark:text-[#E1E2E6]"
                                    style={{ fontFamily: "var(--font-space-grotesk)", letterSpacing: "1.4px" }}
                                >
                                    {p.licenseNumber}
                                </span>
                                <span

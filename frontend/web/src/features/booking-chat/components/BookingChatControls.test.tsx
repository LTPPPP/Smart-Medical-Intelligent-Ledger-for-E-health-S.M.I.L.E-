...
describe("booking chat structured controls", () => {
  ...
  it("renders a focused reschedule selector without a cancel action", async () => {
    ...
    expect(screen.getByRole("button", { name: "Select for reschedule" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Cancel" })).not.toBeInTheDocument();

    await userEvent.click(screen.getByRole("button", { name: "Select for reschedule" }));
    expect(onAction).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("renders appointment metadata as separate readable rows", () => {
    ...
    expect(screen.queryByText(/Dr\. Nguyen Van A • Phòng khám tổng quát 1/)).not.toBeInTheDocument();
  });
  ...
  it("renders doctor choices before time choices and emits the selected doctor", async () => {
    ...
    expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ doctor_id: "doctor-b" }));
  });
      ...
      <AssistantDataCard
        message={{
          ...
          safeState: {
            ...
            booking_options: [
              { id: "slot-1", appointment_time: "10:00", doctor_name: "Dr. An", room_name: "Room 1" },
              { id: "slot-2", appointment_time: "10:30", doctor_name: "Dr. An", room_name: "Room 1" },
            ],
          },
        }}
        isSending={false}

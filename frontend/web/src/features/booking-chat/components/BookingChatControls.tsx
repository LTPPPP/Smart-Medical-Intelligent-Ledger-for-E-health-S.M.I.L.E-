...
export function appointmentLabel(item: AppointmentPreview) {
	const schedule = [item.appointment_date, item.appointment_time]
		.filter(Boolean)
		.join(" at ");
...
export function buildSlotSelectionMessage(
	...
	const intent =
		flow === "reschedule"
			? "Reschedule my selected appointment"
...
export function BookingSlotPicker({
	...
	const groups = Map.groupBy(
		options,
		(item) =>
			[item.doctor_name, item.room_name, item.clinic_name]
				.filter(Boolean)
				.join(" • ") || "SMILE slot",
	);
							...
							return (
								...
								>
									<span className="block font-semibold">
										{item.appointment_time ?? slotLabel(item)}
									</span>
	...
	return (
		<div className="mt-3 space-y-2 rounded-md border border-slate-200 bg-white p-3 text-xs text-slate-700">
			...
			{appointments.slice(0, 4).map((appointment) => (
				<article
					key={appointmentRefOf(appointment)}
				...
				>
					...
					<div
						className={`mt-3 grid gap-2 ${actions.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
					>
						{actions.map((action) => {
							...
							return (
								<button
									...
									disabled={disabled}
									onClick={() =>
										onAction(buildAppointmentAction(action, appointment))
									}
									className={
										action === "reschedule"

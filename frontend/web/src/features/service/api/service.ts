...
export const serviceApi = {
	...
	): Promise<BaseResponse<PaginatedResponse<Specialty>>> => {
		const { data } = await apiClient.get<
			BaseResponse<PaginatedResponse<Specialty>>
		>(API_ENDPOINTS.SPECIALTY.LIST, { params });
		return data;
	},
	...
	getServices: async (
		...
		const { data } = await apiClient.get<
			BaseResponse<PaginatedResponse<Service>>
		>(API_ENDPOINTS.SERVICE.LIST, { params });
		return data;
	},
	...
	searchServices: async (params: {
		...
		const { data } = await apiClient.get<
			BaseResponse<PaginatedResponse<Service>>
		>(API_ENDPOINTS.SERVICE.SEARCH, { params });
		return data;
	},

...
export function useCreateService() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (request: CreateServiceRequest) => managementServiceApi.createService(request),
...
export function useCreateSpecialty() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (request: CreateSpecialtyRequest) => managementServiceApi.createSpecialty(request),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [SPECIALTIES_KEY] }),
  });
}
...
export function useUpdateSpecialty() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ specialtyId, data }: { specialtyId: string; data: UpdateSpecialtyRequest }) =>
...
export function useDeleteSpecialty() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (specialtyId: string) => managementServiceApi.deleteSpecialty(specialtyId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [SPECIALTIES_KEY] }),
  });
}

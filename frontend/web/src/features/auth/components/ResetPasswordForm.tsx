...
export function ResetPasswordForm() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hash) { setError("Missing or invalid reset link. Please request a new reset email."); return; }
    ...
    try {
      ...
      setError("Failed to reset. The link may have expired — request a new reset email.");
    }
  };
            ...
            <div className="px-8 py-9">
              ...
              {!hash && (
                <div className="mb-5 flex items-start gap-2.5 rounded-xl bg-amber-50 px-4 py-3 font-inter text-sm text-amber-700 dark:bg-amber-950/30 dark:text-amber-400">
                  <Icon icon="lucide:link-2-off" width={15} className="mt-0.5 shrink-0" />
                  <span>This reset link is invalid or missing. Please use the link from your reset email.</span>
                </div>
              )}

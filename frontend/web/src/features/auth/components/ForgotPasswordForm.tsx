    ...
    return (
        <div className="group">
            ...
            <div className="flex items-center gap-3 pb-2.5">
                <Icon icon={icon} width={15} className="shrink-0 text-smile-primary/60" />
                <div className="flex-1">{children}</div>
            </div>
            <div className="h-px transition-all duration-200 group-focus-within:bg-smile-primary" style={{ background: "var(--surface-panel-border)" }} />
        </div>
    );
}
                    ...
                    <div key={s.id} className="flex items-center">
                        <div
                            className={
                                ...
                                (done ? "border-smile-primary bg-smile-primary text-white" :
                                    active ? "border-smile-primary bg-smile-primary/10 text-smile-primary shadow-[0_0_12px_rgba(65,126,170,0.3)]" : "")
                            }
                            style={!done && !active ? { background: "var(--surface-panel-bg)", border: "2px solid var(--surface-panel-border)", color: "var(--color-smile-description)" } : undefined}
                        >
                            {done ? <Icon icon="lucide:check" width={13} /> : s.n}
                        </div>
                        <span className={"mx-2 font-inter text-[11px] " + (active ? "font-semibold text-smile-primary" : "text-smile-description")}>
                            {s.label}
                        </span>
                        {i < STEPS.length - 1 && (
                            <div
                                className="mr-2 h-px w-8 rounded-full transition-all duration-500"
                                style={{ background: i < cur ? "var(--color-smile-primary)" : "var(--surface-panel-border)" }}
                            />
                        )}
                    </div>
                );
...
export function ForgotPasswordForm() {
    ...
    const handleSend = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.emailOrPhone) { setError("Please enter your email or phone"); return; }
        try {
            setError("");
            await forgotPassword({ emailOrPhone: form.emailOrPhone });
            setStep("RESET");
        } catch {
            setError("Failed to send OTP. Please check your email or phone and try again.");
        }
    };

    const handleReset = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.otp) { setError("Please enter the OTP code"); return; }
        ...
        try {
            setError("");
            await resetPassword({ emailOrPhone: form.emailOrPhone, otp: form.otp, newPassword: form.newPassword });
            setStep("DONE");
        } catch {
            setError("Failed to reset. Please check your OTP and try again.");
        }
    };
    ...
    return (
        <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background px-4 py-12">
            ...
            >
                <Image src="/images/glassy_tool.png" alt="" width={80} height={80} className="object-contain" />
            </motion.div>
            <motion.div
                animate={{ y: [0, -10, 0] }}
            ...
            >
                <Image src="/images/glassy_block.png" alt="" width={70} height={70} className="object-contain" />
            </motion.div>
            ...
            >
                <Link href={ROUTES.HOME} className="flex items-center gap-2.5">
                    <Image src="/images/logo.png" alt="S.M.I.L.E" width={42} height={42} className="drop-shadow-md" />
                    <span className="font-poppins text-2xl font-semibold tracking-[3px] text-smile-primary">S.M.I.L.E</span>
                </Link>
                <p className="font-inter text-xs text-smile-description">Smart Dental Platform</p>
            </motion.div>
            ...
            <AnimatePresence mode="wait">
                ...
                {step === "DONE" && (
                    ...
                    >
                        ...
                        >
                            <Icon icon="lucide:check-circle" width={46} className="text-green-500" />
                        </motion.div>
                        <h2 className="font-poppins text-2xl font-bold text-smile-primary">All done!</h2>
                        <p className="mt-2 font-inter text-sm leading-relaxed text-smile-description">
                            Your password has been reset.<br />You can now sign in with your new password.
                        </p>
                        <Link
                            href={ROUTES.LOGIN}
                    ...
                    >
                        ...
                        <div className="px-8 py-9">
                            <AnimatePresence mode="wait">
                                ...
                                {/* Step 1: send */}
                                {step === "SEND" && (
                                    <motion.div key="send" variants={fadeUp} initial="hidden" animate="visible" exit="exit">
                                        <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-smile-primary/10">
                                            <Icon icon="lucide:mail-open" width={24} className="text-smile-primary" />
                                        </div>
                                        ...
                                        <form onSubmit={handleSend} className="space-y-7">
                                            ...
                                            >
                                                {isForgotPassword && <Icon icon="line-md:loading-twotone-loop" width={16} />}
                                ...
                                {/* Step 2: reset */}
                                {step === "RESET" && (
                                    <motion.div key="reset" variants={fadeUp} initial="hidden" animate="visible" exit="exit">
                                        <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-smile-primary/10">
                                            <Icon icon="lucide:key-round" width={24} className="text-smile-primary" />
                                        </div>
                                        <h1 className="font-poppins text-[28px] font-bold leading-snug text-smile-primary">
                                            Reset your password
                                        </h1>
                                        <p className="mb-7 mt-2 font-inter text-sm leading-relaxed text-smile-description">
                                            Code sent to{" "}
                                            <span className="font-semibold text-smile-primary">{form.emailOrPhone}</span>.
                                            {" "}Enter it below.
                                        </p>
                                        ...
                                        <form onSubmit={handleReset} className="space-y-5">
                                            ...
                                            <Field label="New Password" icon="lucide:lock">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button type="button" onClick={() => setShowPassword(p => !p)} className="shrink-0 text-smile-description hover:text-smile-primary">
                                                        <Icon icon={showPassword ? "lucide:eye-off" : "lucide:eye"} width={15} />
                                                    </button>
                                                </div>
                                            </Field>
                                            <Field label="Confirm Password" icon="lucide:lock">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button type="button" onClick={() => setShowConfirm(p => !p)} className="shrink-0 text-smile-description hover:text-smile-primary">
                                                        <Icon icon={showConfirm ? "lucide:eye-off" : "lucide:eye"} width={15} />
                                                    </button>
                                                </div>
                                            </Field>
                                            ...
                                            <div className="flex gap-3 pt-1">
                                                ...
                                                >
                                                    {isResettingPassword && <Icon icon="line-md:loading-twotone-loop" width={16} />}

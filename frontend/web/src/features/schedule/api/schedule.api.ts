...
function mapSchedule(raw: ScheduleRaw): DoctorSchedule {
	return {
		...
		).toUpperCase() as DoctorSchedule["status"],
...
export const scheduleApi = {
	getDoctorSchedules: async (
		...
		const res = await api
			.get(API_ENDPOINTS.SCHEDULE.LIST, { params: queryParams })
			.then((r) => r.data);
	...
	getDoctorScheduleById: async (id: string): Promise<DoctorSchedule> => {
		const res = await api
			.get(API_ENDPOINTS.SCHEDULE.DETAIL(id))
			.then((r) => r.data);
	...
	createDoctorSchedule: async (
		...
		const res = await api
			.post(API_ENDPOINTS.SCHEDULE.CREATE, data)
			.then((r) => r.data);
		return mapSchedule(res.data ?? res);
	},
	...
	updateDoctorSchedule: async (
		...
		const res = await api
			.patch(API_ENDPOINTS.SCHEDULE.UPDATE(id), data)
			.then((r) => r.data);
		return mapSchedule(res.data ?? res);
	},
	...
	cancelDoctorSchedule: async (id: string): Promise<DoctorSchedule> => {
		const res = await api
			.patch(API_ENDPOINTS.SCHEDULE.CANCEL(id), {})
			.then((r) => r.data);
		return mapSchedule(res.data ?? res);
	},
	...
	completeDoctorSchedule: async (id: string): Promise<DoctorSchedule> => {
		const res = await api
			.patch(API_ENDPOINTS.SCHEDULE.COMPLETE(id), {})
			.then((r) => r.data);
		return mapSchedule(res.data ?? res);
	},
	...
	getDoctorLeaves: async (
		...
		const res = await api
			.get(API_ENDPOINTS.DOCTOR_LEAVE.LIST, { params: queryParams })
			.then((r) => r.data);
	...
	approveLeave: async (
		...
		const res = await api
			.patch(API_ENDPOINTS.DOCTOR_LEAVE.DETAIL(leaveId), {
				status: "approved",

...
export function ScheduleForm({
  mode, initial, lockDoctor, doctorLabel, submitting, submitLabel, onSubmit, onCancel,
}: {
  mode: 'create' | 'edit';
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-5">
      ...
      <div className="flex justify-end gap-3">
        {onCancel && (
          <button type="button" onClick={onCancel} className="rounded-full border [border-color:var(--surface-input-border)] [background:var(--surface-input-bg)] px-5 py-3 text-sm font-semibold text-smile-title transition hover:border-smile-primary/40">Cancel</button>
        )}
        <button
          type="submit"
          disabled={submitting}

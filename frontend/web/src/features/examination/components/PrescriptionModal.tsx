			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
				<Field label="Prescription date">
					<input
						...
						onChange={(e) => set("prescription_date", e.target.value)}
					/>
				</Field>
			</div>
			<Field label="Notes">
...
export function PrescriptionItemModal({
	...
	const [error, setError] = useState("");
	const set = (
		k: keyof PrescriptionItemFormValues,
		...
		>
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
				...
				<Field label="Medication code">
					<input
						className={inputCls}
						value={form.medication_code ?? ""}
						placeholder="AMOX-500"
						onChange={(e) => set("medication_code", e.target.value)}
					/>
				</Field>
				<Field label="Dosage">
					<input

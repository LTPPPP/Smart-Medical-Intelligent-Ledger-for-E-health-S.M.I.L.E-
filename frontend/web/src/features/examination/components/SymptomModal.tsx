...
export function SymptomModal({
	...
	const set = (k: keyof SymptomFormValues, v: string) =>
		setForm((f) => ({ ...f, [k]: v }));
			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
				<Field label="Symptom name">
					<input
						...
						onChange={(e) => set("symptom_name", e.target.value)}
					/>
				</Field>
				<Field label="Body location">
					<input
						...
						onChange={(e) => set("body_location", e.target.value)}
					/>
				</Field>
				<Field label="Severity">
					...
					>
						{SEVERITY_OPTIONS.map((s) => (
							<option key={s || "none"} value={s} className="bg-[#101922]">
								{s || "—"}
							</option>
						))}
					</select>

...
export function EncounterLegalReminderPanel({
  ...
  const saveRepresentative = useMutation({
    ...
    onSuccess: () => {
      ...
      setRepresentativeForm(emptyRepresentativeForm(representatives.length === 0));
  ...
  const markRead = useMutation({
    mutationFn: () => examinationApi.markAppointmentReminderRead(appointmentId!),
    onSuccess: () => {
      ...
      <section className="flex min-w-0 flex-col gap-4">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              disabled={!hasAppointment || !latestReminderLog || markRead.isPending}
  ...
  return (
    <div className={`rounded-xl p-4 ${panelBase}`}>
      ...
      <div className="mt-2 grid gap-1 text-xs text-smile-description sm:grid-cols-2">
        ...
        <span>Preference: {log.preference_enabled === false ? 'off' : 'on'}</span>
      </div>
      {log.error_message && (
...
function Badge({
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-[11px] capitalize ${toneClass}`}>
      {label}
    </span>
  );
}

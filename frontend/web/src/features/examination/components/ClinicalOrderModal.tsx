...
export function ClinicalOrderModal({
	...
	const set = (k: keyof ClinicalOrderFormValues, v: string) =>
		setForm((f) => ({ ...f, [k]: v }));
		...
		>
				...
				<Field label="Order type">
					...
					>
						{ORDER_TYPE_OPTIONS.map((o) => (
							<option key={o.value} value={o.value} className="bg-[#101922]">
								{o.label}
							</option>
						))}
					</select>
				...
				<Field label="Urgency">
					...
					>
						{URGENCY_OPTIONS.map((u) => (
							<option key={u} value={u} className="bg-[#101922]">
								{u}
							</option>
						))}
					</select>
				...
				<Field label="Status">
					...
					>
						{STATUS_OPTIONS.map((s) => (
							<option key={s} value={s} className="bg-[#101922]">
								{s}
							</option>
						))}
					</select>
			...
			<Field label="Teeth numbers (comma separated)">
				<input
					...
					onChange={(e) => setTeethRaw(e.target.value)}
				/>
			</Field>

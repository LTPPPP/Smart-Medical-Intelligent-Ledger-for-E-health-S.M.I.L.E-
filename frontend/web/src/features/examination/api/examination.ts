...
export const examinationApi = {
	...
	getPatientClinicalContext: async (
		patientId: string,
	...
	updateReminderPreference: async (
		...
		>(API_ENDPOINTS.APPOINTMENT.REMINDER_PREFERENCE(appointmentId), preference);
		return unwrapPayload(data);
	},
	...
	createPrescription: async (
		...
		await Promise.all(
			itemPayloads.map((item) =>
				apiClient.post(PRESCRIPTION_ITEMS_ENDPOINT, {
					...item,
		...
		if (!refreshed.data) {
			throw new Error(
				"Created prescription was not returned by session query.",
			);
		}

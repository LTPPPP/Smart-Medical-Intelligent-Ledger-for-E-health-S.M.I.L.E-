...
const METHODS: { id: Variant; label: string; icon: string; desc: string }[] = [
  { id: 'facility', label: 'At Facility', icon: 'lucide:building-2', desc: 'Book a slot with a doctor at a clinic' },
  { id: 'specialty', label: 'By Specialty', icon: 'lucide:stethoscope', desc: 'Let the clinic assign a specialist' },
  { id: 'doctor', label: 'By Doctor', icon: 'lucide:user-round', desc: 'Choose a specific doctor' },
  { id: 'outside', label: 'Outside Hours', icon: 'lucide:moon', desc: 'Request an after-hours appointment' },
];
...
const EMPTY: FormState = {
  patient_id: '', clinic_id: '', doctor_id: '', specialty_id: '', service_id: '',
  date: '', time: '', chief_complaint: '', notes: '',
};
...
const STEPS = ['Method', 'Patient & Clinic', 'Provider & Schedule', 'Review'] as const;
...
export function BookingWizard() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast.success('Appointment booked'); router.push(ROUTES.APPOINTMENTS); },
    ...
    >
          ...
          return (
            <div key={label} className="flex flex-1 items-center last:flex-none">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className={`hidden font-inter text-[11px] font-semibold sm:block ${active || done ? 'text-smile-primary' : 'text-smile-description'}`}>{label}</span>
              </div>
              {i < STEPS.length - 1 && (
                <div className="mx-2 h-0.5 flex-1 rounded-full" style={{ background: i < step ? 'var(--color-smile-primary)' : 'var(--surface-panel-border)' }} />
              )}
            </div>
          );
        })}
      ...
      {variant === 'outside' && step >= 1 && (
        <div className="flex items-start gap-2 rounded-xl border border-smile-primary/30 bg-smile-primary/5 px-4 py-3 font-inter text-sm text-smile-title">
          <Icon icon="lucide:info" width={16} className="mt-0.5 shrink-0 text-smile-primary" />
          <span>After hours: this slot falls outside the clinic&apos;s normal business hours and may require special staffing approval.</span>
        </div>
      )}
        ...
        >
              ...
              {METHODS.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    type="button"
                    onClick={() => { setVariant(m.id); setError(''); }}
                    className={`flex items-start gap-3 rounded-2xl border p-4 text-left transition-all ${
                      active ? 'border-smile-primary bg-smile-primary-light/50 shadow-[0_4px_16px_rgba(65,126,170,0.15)]' : 'hover:border-smile-primary/40'
                    }`}
                    style={!active ? { background: 'var(--surface-panel-bg)', borderColor: 'var(--surface-panel-border)' } : undefined}
                  >
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${active ? 'bg-smile-primary text-white' : 'bg-smile-primary-light text-smile-primary'}`}>
                      <Icon icon={m.icon} width={20} />
                    </div>
                    <div>
                      <p className="font-poppins text-sm font-semibold text-smile-primary-dark">{m.label}</p>
                      <p className="font-inter text-xs text-smile-description">{m.desc}</p>
                    </div>
                    {active && <Icon icon="lucide:check-circle" width={18} className="ml-auto shrink-0 text-smile-primary" />}
                  </button>
                );
              })}
            </div>
          ...
          {step === 3 && (
            <div className="flex flex-col gap-5">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Field label={variant === 'outside' ? 'Reason (after-hours)' : 'Chief complaint'}>
              ...
              {/* Summary */}
              <div className="rounded-2xl border p-5" style={{ background: 'var(--surface-panel-bg)', borderColor: 'var(--surface-panel-border)' }}>
                <p className="mb-3 font-inter text-[10px] font-semibold uppercase tracking-[2px] text-smile-description">Review</p>
                <dl className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
                  {[
                    ['Method', METHODS.find((m) => m.id === variant)?.label],
                    ['Patient', nameOf.patient],
                    ['Clinic', nameOf.clinic],
                    ...(variant === 'specialty' ? [['Specialty', nameOf.specialty]] : [['Doctor', nameOf.doctor]]),
                    ...(variant !== 'specialty' && form.service_id ? [['Service', nameOf.service]] : []),
                    ['Date', form.date || '—'],
                    ['Time', form.time || '—'],
                    ...(form.chief_complaint ? [['Reason', form.chief_complaint]] : []),
                  ].map(([k, v]) => (
                    <div key={k as string} className="flex items-center justify-between gap-3 border-b py-1.5 last:border-0 [border-color:var(--surface-panel-border)]">
                      <dt className="font-inter text-xs text-smile-description">{k}</dt>
                      <dd className="truncate font-inter text-sm font-medium text-smile-title">{v || '—'}</dd>
                    </div>
                  ))}
                </dl>
              </div>
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        {step < STEPS.length - 1 ? (
          ...
          >
            {createMut.isPending && <Icon icon="line-md:loading-twotone-loop" width={16} />}
            Confirm Booking
          </button>
        )}
      </div>

  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger className={triggerCls}>
        <Icon icon="lucide:calendar-days" width={17} className="shrink-0 text-smile-primary/70" />
...
export function BookingTimePicker({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger className={triggerCls}>
        <Icon icon="lucide:clock" width={17} className="shrink-0 text-smile-primary/70" />
        <span className={cn(!value && 'text-smile-description')}>{value || 'Pick a time'}</span>
      </PopoverTrigger>

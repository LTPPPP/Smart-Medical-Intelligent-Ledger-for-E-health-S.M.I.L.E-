...
describe("CreateAppointmentForm availability picker", () => {
  it("loads server-driven slots and submits the selected option token", async () => {
    vi.mocked(appointmentApi.findAvailability).mockResolvedValue({
      ...
      dates: [{
        ...
        doctors: [{
          ...
          slots: [{ option_token: "slot-token-0900", start_time: "09:00", occupied_until: "09:55" }],
        }],
      }],
    });
    ...
    await userEvent.type(screen.getByPlaceholderText("Enter clinic ID"), "clinic-1");
    await userEvent.type(screen.getByPlaceholderText("Enter doctor ID"), "doctor-1");
    await userEvent.type(screen.getByPlaceholderText("Enter service ID"), "service-1");
    await userEvent.type(container.querySelector('input[type="date"]') as HTMLInputElement, "2026-07-01");
    await userEvent.click(screen.getByRole("button", { name: "Find available slots" }));

  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        {cells.map((day, idx) => {
          if (!day) {
            return <div key={`empty-${idx}`} className="h-24 border-b border-r bg-gray-50" />;
          }
          ...
          return (
            ...
            >
              <span className={`text-xs font-semibold self-start w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-600 text-white' : 'text-gray-700'}`}>
                {day}
              </span>
              ...
              {dayAppointments.length > 2 && (
                <span className="text-[10px] text-gray-500 pl-1">+{dayAppointments.length - 2} more</span>
              )}
            </div>
          );
        })}

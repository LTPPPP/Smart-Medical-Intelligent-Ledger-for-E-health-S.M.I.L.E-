...
export function StaffDashboard({ staffRole }: { staffRole: 'receptionist' | 'nurse' | 'manager' }) {
  ...
  const patients = useMemo(() => unwrapArr<{ patient_id: string }>(patientsRes), [patientsRes]);
  const appointments = useMemo(() => unwrapArr<AppointmentItem>(apptRes), [apptRes]);

  const today = new Date().toISOString().split('T')[0];
  const todayAppts = appointments.filter((a) => (a.appointment_date ?? '').startsWith(today));
  const pending = appointments.filter((a) => a.status === 'pending' || a.status === 'scheduled');
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-8 sm:py-10">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <DashStat label="Today's Appointments" value={todayAppts.length} icon="lucide:calendar-days" loading={apptLoading} accent />
        <DashStat label="Pending / Scheduled" value={pending.length} icon="lucide:clock" loading={apptLoading} />
        <DashStat label="Total Appointments" value={appointments.length} icon="lucide:calendar-check" loading={apptLoading} />
        <DashStat label="Patients" value={patients.length} icon="lucide:users" loading={patientsLoading} />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <DashPanel title="Today's Appointments" icon="lucide:calendar-check">
            {apptLoading ? <DashLoading /> : todayAppts.length === 0 ? <DashEmpty label="No appointments today" /> : (
                ...
                {todayAppts.slice(0, 8).map((a) => (
                  <li key={a.appointment_id} className="flex items-center justify-between gap-4 px-6 py-3.5">
                    <div className="min-w-0">
                      ...
                      <p className="font-inter text-xs text-smile-description">{fmtDate(a.appointment_date)}{a.appointment_time ? ` · ${a.appointment_time}` : ''}</p>
                    </div>
        ...
        <DashPanel title="Quick Actions" icon="lucide:zap">
          <div className="flex flex-col gap-3 p-4">
            {links.map((l) => <DashQuickLink key={l.href} {...l} />)}
          </div>
        </DashPanel>
      </div>
    </div>

...
export function DoctorDashboard() {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ data?: DoctorDashboard } | DoctorDashboard>(API_ENDPOINTS.REPORTS.DASHBOARD_DOCTOR, {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-8 sm:py-10">
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-2xl border p-5 backdrop-blur-xl" style={{ background: 'var(--surface-card-bg)', borderColor: 'var(--surface-card-border)' }}>
        ...
        >
          <Icon icon={isFetching ? 'lucide:loader-2' : 'lucide:refresh-cw'} width={15} className={isFetching ? 'animate-spin' : ''} />
          Refresh
        </button>
      </div>

      {isError && <DashError label="Failed to load doctor dashboard." onRetry={() => refetch()} />}

      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        <DashStat label="Today Total" value={num(summary?.total)} icon="lucide:calendar-days" loading={isLoading} accent />
        <DashStat label="Pending" value={num(summary?.pending)} icon="lucide:clock" loading={isLoading} />
        <DashStat label="Confirmed" value={num(summary?.confirmed)} icon="lucide:badge-check" loading={isLoading} />
        <DashStat label="Completed" value={num(summary?.completed)} icon="lucide:check-circle-2" loading={isLoading} />
        <DashStat label="Cancelled" value={num(summary?.cancelled)} icon="lucide:x-circle" loading={isLoading} />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <DashPanel title="Upcoming Schedules (7 days)" icon="lucide:calendar-clock">
          {isLoading ? <DashLoading /> : schedules.length === 0 ? <DashEmpty label="No upcoming schedules" /> : (
              ...
              {schedules.map((s) => (
                <li key={s.schedule_id} className="flex items-center justify-between gap-4 px-6 py-3.5">
                  <div className="min-w-0">
                    <p className="font-inter text-sm font-medium text-smile-title">{fmtDate(s.work_date)}</p>

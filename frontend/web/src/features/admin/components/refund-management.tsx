...
import type { AdminPayment, RefundStatus } from '@/features/admin/types/admin.type';
...
export function RefundManagement() {
  const { useRefundQueue, approveRefund, rejectRefund, isReviewingRefund } = useAdmin();
  ...
  const handleReject = async () => {
    ...
    setActionError(undefined);
    try {
      await rejectRefund({ id: rejectTarget.payment_id, request: { reason: rejectReason.trim() } });
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {(payments ?? []).map((payment) => (
                <tr key={payment.payment_id} className="hover:bg-slate-50">
                  ...
                  <td className="max-w-[220px] truncate px-4 py-3 text-slate-600" title={payment.refund_reason ?? ''}>
                    {payment.refund_reason || '—'}
                  </td>
                  <td className="px-4 py-3 text-slate-500">
                    {payment.refund_requested_at ? formatDateTime(payment.refund_requested_at) : '—'}
                  </td>
                  <td className="px-4 py-3">
                    {payment.refund_status && (
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${statusClass[payment.refund_status]}`}>

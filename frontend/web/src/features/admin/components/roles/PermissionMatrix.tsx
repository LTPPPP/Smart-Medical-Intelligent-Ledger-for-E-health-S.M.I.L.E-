...
export function PermissionMatrix({
    ...
    const actions = useMemo(() => getAllActions(allPermissions), [allPermissions]);
    ...
    if (isLoadingAll || isLoadingRole) {
        return (
            <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 font-inter text-xs text-smile-description">Loading permissions…</span>
            </div>
        );
    }
        ...
        <div className="space-y-3 px-4 pb-4 pt-3">
            <div className="flex items-center justify-between">
                <p className="font-inter text-xs font-semibold uppercase tracking-wider text-smile-description">
                    Permissions for{' '}
                    <span className="font-bold text-violet-600 dark:text-violet-400">{role.role_name}</span>
                </p>
                <button
                    type="button"
            ...
            {allPermissions.length === 0 ? (
                ...
                >
                    <Icon icon="lucide:key-round" width={24} className="text-smile-description opacity-40" />
                    <p className="font-inter text-xs text-smile-description">No permissions defined in the system yet</p>
                    <button
                        type="button"
                        onClick={onAddPermission}
                ...
                <div className="overflow-x-auto rounded-xl border" style={{ borderColor: 'var(--surface-card-border)' }}>
                    <table className="w-full min-w-[480px]">
                        <thead>
                            <tr
                                className="border-b"
                                style={{ borderColor: 'var(--surface-card-border)', background: 'var(--surface-input-bg)' }}
                            >
                                    ...
                                    >
                                        ...
                                        {actions.map((action) => {
                                            const perm = perms.find(
                                                (p) => (p.action ?? p.permission_name.split('.').pop()) === action,
                                            );
                                            if (!perm) {
                                                return (
                                                    <td key={action} className="px-3 py-2.5 text-center">
                                                        <span className="select-none text-xs text-smile-description opacity-20">—</span>
                                                    </td>
                                                );
                                            }

...
export const adminApi = {
	...
	): Promise<BaseResponse<PaginatedResponse<UserManagement>>> => {
		const { data } = await apiClient.get<
			BaseResponse<PaginatedResponse<UserManagement>>
	...
	getRolePermissions: async (
		roleId: string,
	): Promise<BaseResponse<Permission[]>> => {
	...
	assignUserRole: async (userId: string, roleId: string): Promise<void> => {
		await apiClient.post(API_ENDPOINTS.ADMIN.USER_ROLES.ASSIGN(userId), {
			role_id: roleId,
		});
	},
	...
	revokeUserRole: async (userId: string, roleId: string): Promise<void> => {
		await apiClient.delete(
			API_ENDPOINTS.ADMIN.USER_ROLES.REVOKE(userId, roleId),
		);
	},
	...
	getAllPermissionsV1: async (): Promise<PermissionApi[]> => {
		const { data } = await apiClient.get<PermissionApi[]>(
			API_ENDPOINTS.ADMIN.PERMISSIONS_V1.LIST,
		);
		return data;
	},

	createPermission: async (
		request: CreatePermissionApiRequest,
	): Promise<PermissionApi> => {
	...
	updatePermission: async (
		id: string,
		request: UpdatePermissionApiRequest,
	): Promise<PermissionApi> => {
	...
	assignPermissionToRole: async (
		...
		await apiClient.post(
			API_ENDPOINTS.ADMIN.PERMISSIONS_V1.ASSIGN_TO_ROLE(roleId),
			{

...
const normalizeRecordStatus = (
	...
	const status = String(
		raw.record_status ?? raw.status ?? "draft",
	).toUpperCase();
	if (status === "FINALIZED") return status;
	return "DRAFT";
};

export function mapMedicalRecordForPatient(
	raw: Record<string, unknown>,
): MedicalRecord {
	...
	return {
		...
		prescription: presc
			? {
					medications: (presc.medications as Medication[]) ?? [],
...
export const patientApi = {
	...
	updateMedicalRecord: async (
		...
		const { data } = await api.patch(
			API_ENDPOINTS.MEDICAL_RECORD.UPDATE(id),
			body,
		);
		return normalizeSingle(data, mapMedicalRecordForPatient);
	},
	...
	finalizeMedicalRecord: async (id: string) => {
		const { data } = await api.patch(
			`${API_ENDPOINTS.MEDICAL_RECORD.UPDATE(id)}/finalize`,
		);
		return normalizeSingle(data, mapMedicalRecordForPatient);
	},

...
export function usePatient() {
	...
	const updatePatientMutation = useMutation({
		mutationFn: ({
			id,
			body,
		}: { id: string; body: Partial<Record<string, unknown>> }) =>
	...
	const createRecordMutation = useMutation({
		mutationFn: (body: Partial<Record<string, unknown>>) =>
			patientApi.createMedicalRecord(body),
		onSuccess: (_, body) =>
			invalidateRecords(body.patient_id as string | undefined),
	});

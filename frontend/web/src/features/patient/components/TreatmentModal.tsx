...
export function TreatmentModal({
	...
	const [form, setForm] = useState<{
		...
		...{
			...
			...(initial?.treatment_date
				? { treatment_date: initial.treatment_date.slice(0, 10) }
				: {}),
			...(initial?.procedure_name != null
				? { procedure_name: initial.procedure_name }
				: {}),
			...(initial?.tooth_numbers
				? { tooth_numbers: initial.tooth_numbers.join(", ") }
				: {}),
			...(initial?.procedure_code != null
				? { procedure_code: initial.procedure_code }
				: {}),
	...
	const submit = (e: React.FormEvent) => {
		...
		if (form.procedure_code.trim())
			out.procedure_code = form.procedure_code.trim();
		if (form.cost.trim() && !Number.isNaN(Number(form.cost)))
			out.cost = Number(form.cost);
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
					...
					<Field label="Performed by" required>
						<input
							className={inputCls}
							style={inputStyle}
							value={defaultDoctorLabel ?? form.performed_by}
							readOnly
							disabled
						/>
					</Field>

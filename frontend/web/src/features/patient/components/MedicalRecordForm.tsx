...
export function MedicalRecordForm({
  ...
  const isEdit = !!record;
  const { createMedicalRecord, updateMedicalRecord, isCreatingMedicalRecord, isUpdatingMedicalRecord } =
    usePatient();

  const [visitDate, setVisitDate] = useState(
    record?.visitDate ? record.visitDate.slice(0, 10) : new Date().toISOString().slice(0, 10),
  );
  ...
  const removeMed = (idx: number) => setMedications((m) => m.filter((_, i) => i !== idx));
  const updateMed = (idx: number, key: keyof MedRow, val: string) =>
    setMedications((m) => m.map((row, i) => (i === idx ? { ...row, [key]: val } : row)));

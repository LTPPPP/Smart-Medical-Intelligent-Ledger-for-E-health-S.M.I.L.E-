...
export function MedicalRecordModal({
	...
	const set = (k: keyof MedicalRecordFormValues, v: string) =>
		setForm((f) => ({ ...f, [k]: v }));
	...
	const submit = (e: React.FormEvent) => {
		...
		(
			["chief_complaint", "diagnosis", "treatment_plan", "notes"] as const
		).forEach((k) => {
			...
			<form onSubmit={submit} className="flex flex-col gap-4">
					...
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						...
						<Field label="Doctor" required>
							<input
								className={inputCls}
								style={inputStyle}
								value={defaultDoctorLabel ?? form.doctor_id}
								readOnly
								disabled
							/>
						</Field>
				...
				<Field label="Chief complaint">
					<input
						...
						placeholder="Toothache, swelling…"

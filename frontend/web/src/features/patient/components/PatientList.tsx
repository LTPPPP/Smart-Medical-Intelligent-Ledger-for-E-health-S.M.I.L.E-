...
function PatientDetail({ patient, router }: { patient: Patient; router: ReturnType<typeof useRouter> }) {
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {tab === 'overview' && (
          <div className="grid grid-cols-2 gap-4">
                ...
                {[
                  ...
                  .map((item, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs text-slate-600">
                      <Icon icon={item!.icon} width={14} className="text-teal-600 flex-none" />
            ...
            <div className="col-span-2 sm:col-span-1 bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              {(patient.allergies?.length ?? 0) > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-semibold text-red-500 mb-1.5">Allergies</p>
                  <div className="flex flex-wrap gap-1.5">
        ...
        {tab === 'records' && (
          <div className="flex flex-col items-center justify-center h-40 gap-3">
            <Icon icon="mdi:file-document-outline" width={40} className="text-slate-300" />
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {isLoading && (
            <div className="space-y-2 p-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="rounded-xl p-3 animate-pulse bg-slate-50">
          ...
          {!isLoading && !error && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-slate-400">
              <Icon icon="mdi:account-multiple-outline" width={40} className="mb-2 text-slate-300" />
            ...
            filtered.map((patient) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-900 text-sm truncate">{patient.fullName}</p>
        ...
        {!selectedId || !selectedPatient ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-teal-50 flex items-center justify-center">
              <Icon icon="mdi:account-details" width={36} className="text-teal-400" />
            </div>
            <div className="text-center">
              <p className="font-semibold text-slate-600">Select a patient</p>
              <p className="text-sm text-slate-400 mt-1">Click a name on the left to view details</p>
            </div>
          </div>
        ) : (

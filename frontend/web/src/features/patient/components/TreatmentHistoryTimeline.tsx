...
export function TreatmentHistoryTimeline({ patientId }: TreatmentHistoryTimelineProps) {
              ...
              <div className="bg-white rounded-2xl shadow-[4px_4px_10px_rgba(177,192,202,0.6),-4px_-4px_10px_rgba(255,255,255,1)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-slate-800 text-sm">{treatment.procedure}</p>
                    ...
                    {treatment.description && (
                      <p className="text-xs text-slate-500 mt-1.5">{treatment.description}</p>
                    )}

                    <p className="text-xs text-slate-400 mt-1.5 flex items-center gap-1.5">
                      <Icon icon="mdi:calendar" width={12} />
                      {new Date(treatment.treatmentDate).toLocaleDateString('vi-VN')}

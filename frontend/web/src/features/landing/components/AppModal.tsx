...
const ROLE_FEATURES: AppFeature[] = [
  { label: 'Admin Panel', href: ROUTES.ADMIN, icon: 'lucide:shield', color: 'indigo', requiredRoles: ADMIN_ROLES },
  { label: 'Patients', href: ROUTES.PATIENTS, icon: 'lucide:users', color: 'blue', requiredRoles: PATIENT_DIRECTORY_ROLES },
  { label: 'Examinations', href: ROUTES.EXAMINATIONS, icon: 'lucide:clipboard-check', color: 'emerald', requiredRoles: EXAMINATION_ROLES },
  { label: 'Dental Images', href: ROUTES.DENTAL_IMAGES, icon: 'lucide:scan', color: 'cyan', requiredRoles: DENTAL_IMAGE_ROLES },
  { label: 'My Performance', href: ROUTES.PERFORMANCE, icon: 'lucide:trending-up', color: 'orange', requiredRoles: PERFORMANCE_ROLES },
  { label: 'My Schedule', href: ROUTES.MY_SCHEDULE, icon: 'lucide:calendar-days', color: 'purple', requiredRoles: MY_SCHEDULE_ROLES },
  { label: 'Doctor Schedules', href: ROUTES.DOCTOR_SCHEDULES, icon: 'lucide:calendar-cog', color: 'teal', requiredRoles: SCHEDULE_MANAGEMENT_ROLES },
  { label: 'Leave Requests', href: ROUTES.DOCTOR_LEAVES, icon: 'lucide:calendar-off', color: 'pink', requiredRoles: LEAVES_ROLES },
];
...
export function AppModal({ open, onOpenChange }: AppModalProps) {
  ...
  const roleFeatures = useMemo(() => {
    ...
    return ROLE_FEATURES.filter(
      (feature) => !feature.requiredRoles || feature.requiredRoles.some((role) => user.roles.includes(role)),
    );
  }, [user]);

  const allFeatures = useMemo(() => [...GENERAL_FEATURES, ...roleFeatures], [roleFeatures]);

  const searchResults = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return [];
    return allFeatures.filter((feature) => feature.label.toLowerCase().includes(q));
  ...
  return (
    ...
    >
      ...
      >
        {/* Search bar */}
        <div className="p-4" style={{ borderBottom: '1px solid var(--surface-panel-border)' }}>
          <div
          ...
          >
            <Icon icon="lucide:search" width={18} className="shrink-0 text-smile-description" />
            <input
              ref={inputRef}
              type="text"
              value={searchQuery}
            ...
            />
            <kbd className="ml-auto hidden shrink-0 rounded px-1.5 py-0.5 font-mono text-[10px] text-smile-description sm:block" style={{ background: 'var(--surface-panel-bg)' }}>
              Esc
            </kbd>
          </div>
        </div>
              ...
              {searchResults.length > 0 ? (
                <div className="space-y-1.5">
                  {searchResults.map((feature) => (
                    ...
                    >
                      <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-lg', CATEGORY_STYLES[feature.color])}>
                      ...
                      <Icon icon="lucide:chevron-right" width={16} className="ml-auto text-smile-description opacity-0 transition-all group-hover:translate-x-1 group-hover:opacity-100" />
                    </Link>
                  ))}
                </div>
              ) : (
                <div className="py-10 text-center">
                  <p className="font-black text-[10px] uppercase tracking-[0.2em] text-smile-description">No matching feature</p>
                  <p className="mt-2 font-inter text-xs text-smile-description">Try a different keyword</p>
                </div>
              )}
            </div>
          ) : (
                ...
                <div className="grid grid-cols-2 gap-2">
                  {GENERAL_FEATURES.map((feature) => (
                    <Link
                      ...
                      style={{ background: 'var(--surface-panel-bg)' }}
                    >
                      <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-lg', CATEGORY_STYLES[feature.color])}>
        ...
        <div
          ...
          style={{ borderTop: '1px solid var(--surface-panel-border)', background: 'var(--surface-panel-bg)' }}
        >

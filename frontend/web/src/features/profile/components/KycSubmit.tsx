...
const STATUS_CONFIG = {
	NOT_SUBMITTED: {
		...
		bg: "bg-slate-50 border-slate-200 dark:bg-slate-900/20 dark:border-slate-700",
		label: "Not Submitted",
		desc: "Submit your ID to verify your identity.",
	},
	PENDING_REVIEW: {
		icon: "lucide:clock",
		...
		desc: "Your submission is under review. We'll notify you once complete.",
	},
	VERIFIED: {
	...
	REJECTED: {
		...
		label: "Rejected",
		desc: "Your submission was rejected. Please resubmit with correct documents.",
	},
} as const;
...
function FileDropZone({
	...
	return (
		<div>
			<p className="mb-1.5 font-inter text-xs font-semibold uppercase tracking-[1.5px] text-smile-description">
				{label}
			</p>
			<button
				type="button"
				...
				style={{
					...
					background: file ? "var(--surface-footer-bg)" : undefined,
				}}
			>
				{file ? (
					<>
...
function ConsentRow({
	...
	return (
		<label className="flex cursor-pointer items-start gap-3">
			<span
				className={`relative mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors ${checked ? "border-smile-primary bg-smile-primary" : "border-gray-300 dark:border-white/20"}`}
			>
				<input
					type="checkbox"
					checked={checked}
...
export function KycSubmit() {
	...
	const allConsents =
		consents.main && consents.storage && consents.ocr && consents.noMarketing;
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!idFront || !idBack) {
			setMsg({
				type: "error",
				text: "Please upload both front and back of your ID.",
			});
			return;
		}
		if (!allConsents) {
			setMsg({ type: "error", text: "Please accept all consents to proceed." });
			return;
		}
		if (!/^\d{12}$/.test(form.idNumber)) {
			setMsg({ type: "error", text: "ID number must be exactly 12 digits." });
			return;
		}
		try {
			...
			setMsg({
				type: "success",
				text: "KYC submitted successfully! Your documents are under review.",
			});
	...
	return (
		<div className="space-y-5">
			...
			<div
				className={`flex items-start gap-4 rounded-2xl border p-5 ${cfg.bg}`}
			>
				<Icon
					icon={cfg.icon}
					width={28}
				...
				<div className="min-w-0">
					<p className={`font-poppins text-sm font-semibold ${cfg.color}`}>
						{cfg.label}
					</p>
					<p className="mt-0.5 font-inter text-xs text-smile-title">
						{cfg.desc}
					</p>
					...
					{status === "VERIFIED" && (
						<div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 font-inter text-xs text-smile-description">
							{kyc?.fullName && (
								<span>
									<span className="font-semibold">Name:</span> {kyc.fullName}
								</span>
							)}
							{kyc?.idNumberMasked && (
								<span>
			...
			{canSubmit && (
				<form onSubmit={handleSubmit} className="space-y-5">
					{msg && (
						<div
							className={`flex items-center gap-2 rounded-xl px-4 py-3 font-inter text-sm ${msg.type === "success" ? "bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400" : "bg-red-50 text-red-600 dark:bg-red-950/30 dark:text-red-400"}`}
						>
							<Icon
								icon={
									msg.type === "success"
					...
					<div>
						...
						<div
							className="inline-flex items-center gap-2 rounded-full border bg-smile-primary-light px-4 py-2 font-inter text-sm font-semibold text-smile-primary"
							style={{ borderColor: "var(--color-smile-primary)" }}
						>
					...
					>
						...
						<ConsentRow
							...
							onChange={(v) => setConsents({ ...consents, main: v })}
						>
							I consent to S.M.I.L.E processing my identity information for KYC
						...
						>
							I allow S.M.I.L.E to securely store my uploaded identity document
							images for manual review.
						</ConsentRow>
						<ConsentRow
							checked={consents.ocr}
							onChange={(v) => setConsents({ ...consents, ocr: v })}
						>
							I allow OCR processing of my identity documents to support
							verification.
						</ConsentRow>
						<ConsentRow
							checked={consents.noMarketing}
							onChange={(v) => setConsents({ ...consents, noMarketing: v })}
						>
							I acknowledge that my KYC data will not be used for marketing
							purposes.
						</ConsentRow>
					</div>

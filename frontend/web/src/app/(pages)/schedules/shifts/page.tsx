...
export default function WorkShiftsPage() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: SHIFT_KEY });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(API_ENDPOINTS.WORK_SHIFT.DELETE(id)),
    onSuccess: () => {
  ...
  const handleDelete = (s: WorkShift) => {
    if (window.confirm(`Delete work shift "${s.shift_name}"? This cannot be undone.`)) {
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-8 sm:py-10">
        ...
        {isLoading && (
          <div className={`${cardBase} flex items-center justify-center gap-2 py-16 text-smile-description`}>
            <Icon icon="line-md:loading-twotone-loop" width={20} /> Loading work shifts…
          </div>
        )}
        ...
        {!isLoading && !isError && shifts.length > 0 && (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {shifts.map((s) => {
              const isDeleting = deletingId === s.shift_id && deleteMutation.isPending;
              return (
                <div key={s.shift_id} className={`${cardBase} flex flex-col gap-4 p-6`}>
                  <div className="flex items-start gap-4">
                    <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[20px] border [background:var(--surface-panel-bg)] [border-color:var(--surface-panel-border)]">
                      <Icon icon="lucide:calendar-clock" width={22} className="text-smile-primary" />
                    </span>

  ...
  return (
    <AppShell>
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8 sm:px-8 sm:py-10">
        <div>
          <h1 className="text-[28px] font-bold tracking-[-0.6px] text-smile-primary-dark font-poppins">Schedule Management</h1>
          <p className="text-sm text-smile-description">Manage work schedules, personal schedules and leave.</p>
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {CARDS.map((c) => (
            <Link key={c.href} href={c.href} className={`${cardBase} group flex flex-col gap-3 p-6 transition hover:border-smile-primary/40`}>
              ...
              <h3 className="text-[18px] font-semibold text-smile-title font-poppins">{c.title}</h3>
              <p className="text-sm leading-[23px] text-smile-description">{c.desc}</p>

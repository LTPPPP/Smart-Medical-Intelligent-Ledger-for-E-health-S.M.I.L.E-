...
export default function DentalImagesPage() {
	...
	const { data: recordsRes } = useQuery({
		...
		queryFn: () =>
			apiClient.get(`${GATEWAY}/medical-records/patient/${patientId}`),
		enabled: !!patientId,
	});
	const {
		data: imagesRes,
		...
		queryFn: () =>
			apiClient.get(`${GATEWAY}/dental-images/patient/${patientId}`),
		enabled: !!patientId,
	});

	const patients = useMemo(
		() => unwrapArr<PatientLite>(patientsRes),
		[patientsRes],
	);
	const categories = useMemo(
		() => unwrapArr<CategoryLite>(categoriesRes),
		[categoriesRes],
	);
	const records = useMemo(
		() => unwrapArr<MedicalRecordLite>(recordsRes),
		[recordsRes],
	);
					...
					<div className="grid grid-cols-1 gap-4">
						...
						{editing && (
							<EditImageModal
								...
								onSubmit={(v) =>
									updateImage.mutate({ id: editing.image_id, v })
								}
							/>
						)}
						...
						{categoriesOpen && (
							<CategoryModal onClose={() => setCategoriesOpen(false)} />
						)}
					</div>
				)}
						...
						{images.map((img) => (
							...
							>
								...
								<div className="flex flex-1 flex-col gap-2 p-4">
									{img.description && (
										<p className="text-sm font-medium text-smile-title">
											{img.description}
										</p>
									)}
									<div className="flex flex-col gap-1 text-xs text-smile-description">
										...
										{categoryName(img.category_id) && (
											<span
												className="inline-flex w-fit items-center gap-1 rounded-full border px-2 py-0.5 [border-color:var(--surface-input-border)] [background:var(--surface-input-bg)]"
												style={{ color: TEAL }}
											>
												<Icon icon="lucide:tag" width={11} />{" "}
												{categoryName(img.category_id)}
											</span>
										)}
										{img.record_id && (
										...
										<span className="mt-1">
											{actorName(img.uploaded_by)}
											{img.created_at ? ` · ${fmtDate(img.created_at)}` : ""}
										</span>
									</div>
								</div>

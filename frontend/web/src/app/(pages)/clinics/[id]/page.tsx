...
const panelStyle = { background: 'var(--surface-panel-bg)', borderColor: 'var(--surface-panel-border)' };
  ...
  return (
    <AppShell>
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-8 py-10">
        ...
        <div className="flex items-center justify-between">
          ...
          {clinic && (
            <div className="flex items-center gap-2">
              <Link href={ROUTES.CLINIC_EDIT(clinic.clinic_id)} className="flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold text-smile-title transition hover:opacity-80" style={panelStyle}>
        ...
        {isLoading && (
          <div className={`${cardBase} flex items-center justify-center gap-2 py-20 text-smile-description`} style={cardStyle}>
            <Icon icon="line-md:loading-twotone-loop" width={20} /> Loading…
          </div>
        )}
        {!isLoading && !clinic && (
          <div className={`${cardBase} p-10 text-center text-sm text-smile-description`} style={cardStyle}>Clinic not found.</div>
        )}

        {clinic && (
          <>
            {/* Header card */}
            <div className={`${cardBase} flex flex-col gap-5 p-6`} style={cardStyle}>
              <div className="flex items-start gap-4">
                <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[20px] border" style={panelStyle}>
                  <Icon icon="lucide:building-2" width={26} style={{ color: BLUE }} />
                </span>
                <div className="flex flex-1 flex-col gap-2">
                  <h1 className="text-[26px] font-bold tracking-[-0.5px] text-smile-title font-poppins">{clinic.clinic_name}</h1>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border px-2.5 py-0.5 font-mono text-xs font-semibold" style={{ ...panelStyle, color: TEAL }}>{clinic.clinic_code}</span>
            ...
            {/* Operating hours */}
            <div className={`${cardBase} flex flex-col gap-3 p-6`} style={cardStyle}>
              <h2 className="text-[16px] font-semibold text-smile-title font-poppins">Operating hours</h2>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {DAYS.map((d) => {
                  const t = clinic.operating_hours?.[d];
                  return (
                    <div key={d} className="flex items-center justify-between rounded-lg border px-3 py-2 text-sm" style={panelStyle}>
                      <span className="text-smile-description">{DAY_LABEL[d]}</span>
                      <span className={`font-medium ${t ? 'text-smile-title' : 'text-smile-description'}`}>{t ? `${t.open} – ${t.close}` : 'Closed'}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Treatment rooms */}
            <div className={`${cardBase} flex flex-col gap-4 p-6`} style={cardStyle}>
              <div className="flex items-center justify-between">
              ...
              {rooms.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {rooms.map((r) => (
                    <div key={r.room_id} className="group flex items-center gap-3 rounded-xl border p-3" style={panelStyle}>
                      <span className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ background: 'var(--surface-card-bg)' }}>

    ...
    <AppShell>
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-8 sm:py-10">
        ...
        {!isLoading && !isError && clinics.length === 0 && (
          <div className={`${cardBase} p-10 text-center text-sm text-smile-description`}>No clinics found.</div>
        )}
                ...
                <div key={c.clinic_id} className={`${cardBase} flex flex-col gap-4 p-6`}>
                  ...
                  <div className="flex items-start gap-4">
                    <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[20px] border [background:var(--surface-panel-bg)] [border-color:var(--surface-panel-border)]">
                      <Icon icon="lucide:building-2" width={22} className="text-smile-primary" />
                    </span>
                  ...
                  <div className="flex flex-col gap-2.5 text-sm text-smile-description">
                    <div className="flex items-start gap-2.5">
                      <Icon icon="lucide:map-pin" width={16} className="mt-0.5 shrink-0 text-smile-primary" />
                      <span>{fullAddress || '—'}</span>
                    </div>
                    <div className="flex items-center gap-2.5">
                      <Icon icon="lucide:phone" width={16} className="shrink-0 text-smile-primary" />
                      <span>{c.phone || '—'}</span>
                    </div>
                    <div className="flex items-center gap-2.5">
                      <Icon icon="lucide:mail" width={16} className="shrink-0 text-smile-primary" />
                      <span>{c.email || '—'}</span>
                    </div>
                    <div className="flex items-center gap-2.5">
                      <Icon icon="lucide:clock" width={16} className="shrink-0 text-smile-primary" />
                  ...
                  <div className="flex items-center justify-between border-t [border-color:var(--surface-panel-border)] pt-4">
                    <span className="text-xs text-smile-description">License: {c.license_number || '—'}</span>
                    <Link

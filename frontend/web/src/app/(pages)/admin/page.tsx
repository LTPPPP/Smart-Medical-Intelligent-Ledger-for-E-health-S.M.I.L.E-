...
const getActionMeta = (action: string) =>
	ACTION_META[action] ?? {
		label: action,
		color: "text-smile-description",
		bg: "bg-smile-description/10",
...
const checkStyle: Record<
	KycCheckStatus,
	{ icon: string; badge: string; row: string }
> = {
	PASS: {
...
export default function AdminPage() {
	...
	const { data: selectedKyc, isLoading: isLoadingKycDetail } =
		useKycReview(selectedKycId);
	...
	const chartSeries = useMemo(() => {
		...
		const byDay: Record<
			string,
			{ date: string; LOGIN: number; LOGOUT: number; REGISTER: number }
		> = {};
		for (const log of logs) {
			const day = format(startOfDay(parseISO(log.created_at)), "MM/dd");
			if (!byDay[day])
				byDay[day] = { date: day, LOGIN: 0, LOGOUT: 0, REGISTER: 0 };
	...
	const selectedKycVisibleChecks = selectedKycChecks.filter(
		(check) => check.status !== "PASS",
	);
	const selectedKycReviewChecks =
		selectedKycVisibleChecks.length > 0
			? selectedKycVisibleChecks
			: selectedKycChecks.slice(0, 3);
	...
	const selectedKycAdditionalFields = getAdditionalOcrFields(
		selectedKyc?.ocrPayload,
	);
	const selectedKycTechnicalPayload = getTechnicalOcrPayload(
		selectedKyc?.ocrPayload,
	);
		...
		<div className="space-y-6">
			...
			<motion.div {...fadeUp(0)}>
				...
				>
					...
					/>
					<div
						className="pointer-events-none absolute -bottom-10 right-0 h-56 w-56 rounded-full"
						style={{
							background:
								"radial-gradient(circle, rgba(65,126,170,0.18) 0%, transparent 70%)",
						}}
					/>
					...
					{(kycReviews?.data?.length ?? 0) === 0 ? (
						...
						>
							{kycReviews?.data.map((record) => (
								<div
									key={record.kycId}
									className="grid grid-cols-[1fr_150px_130px_120px] items-center gap-4 px-6 py-3"
								>
									...
									<span className="font-inter text-xs text-smile-description">
										{record.submittedAt
											? format(parseISO(record.submittedAt), "dd/MM HH:mm")
											: "—"}
									</span>
						...
						<ResponsiveContainer width="100%" height={220}>
							...
							>
								...
								<XAxis
									...
									tick={{
										fontSize: 11,
										fill: "var(--color-smile-description, #94a3b8)",
									}}
								/>
								<YAxis
									allowDecimals={false}
					...
					{auditLoading ? (
						...
						>
							{logs.map((log) => {
								...
								const timeStr = (() => {
									try {
										return format(parseISO(log.created_at), "dd/MM HH:mm:ss");
									} catch {
								...
								const email =
									...
									typeof log.details?.email === "number"
								...
								return (
									...
									>
										...
										{/* UA */}
										<p
											className="truncate font-inter text-xs text-smile-description"
											title={ua}
										>
											{shortUa}
										</p>
						...
						<div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
							<div>
								...
								<p className="font-inter text-xs text-slate-500">
									{selectedKyc?.kycId ?? selectedKycId}
								</p>
							</div>
							<button
								type="button"
								...
								>
									<div className="flex flex-wrap items-start justify-between gap-3">
											...
											<div>
												<p className="font-semibold">
													{selectedKycNeedsAttention
														...
														: "OCR signals look consistent"}
												</p>
												<p className="mt-1 text-xs leading-5">
													Review identity documents only for KYC and booking
													safety. OCR is a helper signal, not automatic
													approval.
												</p>
											</div>
										</div>
										<span className="rounded-full bg-white/70 px-3 py-1 text-xs font-semibold">
											Retains until{" "}
											{selectedKyc?.retentionExpiresAt
												? format(
														parseISO(selectedKyc.retentionExpiresAt),
														"dd/MM/yyyy",
													)
												: "policy expiry"}
										</span>
								...
								<div className="grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
										...
										<div className="rounded-xl border border-slate-200 bg-white p-4">
											<div className="mb-3 flex items-center justify-between gap-3">
												<div>
													...
													<p className="font-inter text-xs text-slate-500">
														Showing warnings and failures first. Passing checks
														stay summarized.
													</p>
												</div>
											...
											<div className="mb-3 grid grid-cols-3 gap-2">
												{(
													[
														[
															"PASS",
															selectedKycCheckCounts.PASS,
															"text-green-700 bg-green-50",
														],
														[
															"WARNING",
															selectedKycCheckCounts.WARNING,
															"text-amber-700 bg-amber-50",
														],
														[
															"FAIL",
															selectedKycCheckCounts.FAIL,
											...
											{selectedKycChecks.length === 0 ? (
												<p className="font-inter text-sm text-slate-500">
													No structured checks are available for this request.
												</p>
											) : (
												<div className="grid gap-2">
													...
													{selectedKycVisibleChecks.length === 0 &&
														selectedKycChecks.length >
															selectedKycReviewChecks.length && (
											...
											<div className="space-y-4 p-4">
												<div className="grid gap-3 md:grid-cols-2">
													{[
														...
														[
															"Date of birth",
															getOcrField(selectedKyc, "dateOfBirth"),
														],
														[
															"Document type",
															getOcrField(selectedKyc, "documentType"),
														],
														[
															"Issue date",
															getOcrField(selectedKyc, "issueDate"),
														],
														[
															"Expiry date",
															getOcrField(selectedKyc, "expiryDate"),
														],
														[
															"Origin",
															getOcrField(selectedKyc, "placeOfOrigin"),
														],
														[
															"Residence",
															getOcrField(selectedKyc, "placeOfResidence"),
														],
													].map(([label, value]) => (
														<div
															key={label}
									...
									<aside className="space-y-4">
										<div className="rounded-xl border border-slate-200 p-4">
											<p className="mb-3 font-inter text-sm font-semibold text-slate-800">
												Submitted Data
											</p>
											<dl className="space-y-2 font-inter text-sm">
												...
												<div className="flex justify-between gap-4">
													...
													<dd className="font-medium text-slate-900">
														{selectedKyc?.idNumberMasked ?? "—"}
													</dd>
												</div>
												<div className="flex justify-between gap-4">
													<dt className="text-slate-500">Full name</dt>
													<dd className="max-w-44 truncate font-medium text-slate-900">
												...
												<div className="flex justify-between gap-4">
													...
													<dd className="font-medium text-slate-900">
														{selectedKyc?.submittedAt
															? format(
																	parseISO(selectedKyc.submittedAt),
																	"dd/MM/yyyy HH:mm",
																)
															: "—"}
													</dd>
										...
										<div className="rounded-xl border border-slate-200 p-4">
											...
											<dl className="space-y-2 font-inter text-sm">
												<div className="flex justify-between gap-4">
													<dt className="text-slate-500">Status</dt>
													<dd className="font-medium text-slate-900">
														{selectedKyc?.ocrStatus ?? "—"}
													</dd>
												</div>
												<div className="flex justify-between gap-4">
													<dt className="text-slate-500">Confidence</dt>
													<dd className="font-medium text-slate-900">
														{typeof selectedKyc?.ocrConfidence === "number"
															? `${selectedKyc.ocrConfidence}%`
															: "—"}
													</dd>
												</div>
												<div className="flex justify-between gap-4">
													...
													<dd className="font-medium text-slate-900">
														{selectedKyc?.ocrProcessedAt
															? format(
																	parseISO(selectedKyc.ocrProcessedAt),
																	"dd/MM/yyyy HH:mm",
																)
															: "—"}
													</dd>
										...
										<div className="rounded-xl border border-slate-200 p-4">
											<label
												className="mb-2 block font-inter text-sm font-semibold text-slate-800"
										...
										<div className="flex gap-3">
											...
											<button
												type="button"
												disabled={
													isReviewingKyc || rejectReason.trim().length < 3
												}
												onClick={async () => {
													const reason = rejectReason.trim();
													if (reason.length < 3) {
														setReviewError(
															"Reject reason must be at least 3 characters.",
														);
														return;
													}
													try {
				...
				<div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/70 px-4 py-8 backdrop-blur-sm">
					<div className="grid max-h-[92vh] w-full max-w-6xl overflow-hidden rounded-2xl bg-white shadow-2xl lg:grid-cols-[1.35fr_0.65fr]">
						...
						<aside className="max-h-[92vh] overflow-y-auto p-5">
							<p className="mb-4 font-poppins text-base font-semibold text-slate-900">
								Submitted Data
							</p>
							...
							{selectedKycRiskLevel && (
								<div className="mt-4 rounded-xl border border-slate-200 p-3">
									...
									<p className="mt-1 font-inter text-sm font-bold text-slate-900">
										{selectedKycRiskLevel}
									</p>
									{selectedKycRiskReason && (
										<p className="mt-2 font-inter text-xs leading-5 text-slate-600">
											{selectedKycRiskReason}
										</p>
									)}
								</div>

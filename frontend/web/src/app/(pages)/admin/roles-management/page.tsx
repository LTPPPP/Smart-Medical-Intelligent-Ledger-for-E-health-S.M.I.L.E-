...
import { fadeUpVariants, expandVariants } from '@/features/admin/animations/variants';
...
import type { RoleApi, CreatePermissionApiRequest } from '@/features/admin/types/admin.type';
...
export default function AdminRolesManagementPage() {
  ...
  const handleCreateRole = async (name: string, description: string) => {
    await createRoleApi({ role_name: name, description: description || undefined });
    refetch();
  };
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="relative flex items-center justify-between px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-inter text-xs text-smile-description">{total} role{total !== 1 ? 's' : ''} — click a row to manage permissions</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setShowCreatePermission(true)} className="flex items-center gap-2 rounded-xl border px-4 py-2 font-inter text-sm font-semibold text-violet-600 transition-all hover:bg-violet-50 dark:hover:bg-violet-900/20" style={{ borderColor: 'var(--surface-card-border)' }}>
                <Icon icon="lucide:key-round" width={15} />
                New Permission
              </button>
              <button type="button" onClick={() => setShowCreateRole(true)} className="flex items-center gap-2 rounded-xl bg-smile-primary px-4 py-2 font-inter text-sm font-semibold text-white transition-all hover:bg-smile-primary/90 hover:shadow-[0_4px_14px_rgba(65,126,170,0.4)]">
      ...
      <motion.div variants={fadeUpVariants} initial="hidden" animate="visible" custom={2}>
        <div className="overflow-hidden rounded-[22px] border backdrop-blur-xl" style={{ background: 'var(--surface-card-bg)', borderColor: 'var(--surface-card-border)', boxShadow: 'var(--surface-card-shadow)' }}>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-16">
              ...
              <button type="button" onClick={() => refetch()} className="rounded-lg bg-red-50 px-3 py-1.5 text-xs text-red-600 hover:bg-red-100">Retry</button>
            </div>
          ) : roles.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 py-16">
              <Icon icon="lucide:shield-off" width={32} className="text-smile-description opacity-40" />
              <p className="font-inter text-sm text-smile-description">{debouncedSearch ? `No roles match "${debouncedSearch}"` : 'No roles found'}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b" style={{ borderColor: 'var(--surface-card-border)' }}>
                    <th className="w-10 px-3 py-3.5" />
                    <th className="px-5 py-3.5 text-left font-inter text-[11px] font-semibold uppercase tracking-wider text-smile-description">Role Name</th>
                    <th className="px-5 py-3.5 text-left font-inter text-[11px] font-semibold uppercase tracking-wider text-smile-description">Description</th>
                    <th className="px-5 py-3.5 text-left font-inter text-[11px] font-semibold uppercase tracking-wider text-smile-description">Created</th>
                      ...
                      <React.Fragment key={role.role_id}>
                        ...
                        >
                          <td className="px-3 py-3.5">
                            <button type="button" onClick={() => toggleExpand(role.role_id)} className="flex h-6 w-6 items-center justify-center rounded-md text-smile-description transition-all hover:bg-violet-100 hover:text-violet-600 dark:hover:bg-violet-900/30" title={isExpanded ? 'Collapse permissions' : 'Expand permissions'}>
                          ...
                          <td className="px-5 py-3.5">
                            <button type="button" onClick={() => toggleExpand(role.role_id)} className="flex items-center gap-1.5">
                              <span className="inline-flex items-center gap-1.5 rounded-lg bg-violet-100 px-2.5 py-1 font-inter text-xs font-semibold uppercase tracking-wide text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
                                <Icon icon="lucide:shield-half" width={11} />{role.role_name}
                              </span>
                            </button>
                          </td>
                          <td className="px-5 py-3.5">
                            <span className="font-inter text-sm text-smile-description line-clamp-2">{role.description ?? <span className="italic opacity-50">No description</span>}</span>
                          </td>
                          <td className="px-5 py-3.5">
                            <span className="font-inter text-sm text-smile-description">{formatDate(role.created_at)}</span>
                          </td>
                          <td className="px-5 py-3.5">
                            <button type="button" onClick={() => handleDeleteRole(role)} disabled={isDeletingRole} className="rounded-lg p-1.5 text-red-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:opacity-50 dark:hover:bg-red-900/20" title="Delete role">
                        ...
                        <AnimatePresence>
                          {isExpanded && (
                            <tr className="border-b" style={{ borderColor: 'var(--surface-card-border)' }}>
                              <td colSpan={5} className="p-0" style={{ background: 'var(--surface-input-bg)' }}>
                                <motion.div key={`${role.role_id}-exp`} variants={expandVariants} initial="hidden" animate="visible" exit="exit" style={{ overflow: 'hidden' }}>
                                  <div className="ml-3 border-l-2 border-violet-400">
                                    <RoleExpandedSection role={role} onAddPermission={() => setShowCreatePermission(true)} />
                                  </div>
                                </motion.div>
                              </td>
                            </tr>
          ...
          {!isLoading && !error && total > LIMIT && (
            <div className="flex items-center justify-between border-t px-5 py-3" style={{ borderColor: 'var(--surface-card-border)' }}>
              <p className="font-inter text-xs text-smile-description">Showing {(page - 1) * LIMIT + 1}–{Math.min(page * LIMIT, total)} of {total}</p>
      ...
      <AnimatePresence>
        {showCreateRole && (
          <CreateRoleDialog isLoading={isCreatingRole} onClose={() => setShowCreateRole(false)} onCreate={handleCreateRole} />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {showCreatePermission && (
          <CreatePermissionDialog isLoading={isCreatingPermission} onClose={() => setShowCreatePermission(false)} onCreate={handleCreatePermission} />
        )}
      </AnimatePresence>
    </div>
  );

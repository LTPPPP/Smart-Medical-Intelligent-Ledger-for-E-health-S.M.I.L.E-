...
import { fadeUpVariants, expandVariants, rowVariants } from '@/features/admin/animations/variants';
import {
...
import { formatDateTime, formatRelativeTime } from '@/features/admin/utils/date.utils';
  ...
  return (
    <div className="space-y-3 py-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <p className="font-inter text-[10px] font-semibold uppercase tracking-wider text-smile-description">Resource ID</p>
          <p className="truncate font-mono text-xs text-smile-title">{log.resource_id ?? '—'}</p>
        </div>
        <div>
          <p className="font-inter text-[10px] font-semibold uppercase tracking-wider text-smile-description">Device</p>
          <p className="font-inter text-xs text-smile-title" title={log.user_agent ?? undefined}>
      ...
      {log.details != null && (
        <div>
          <p className="mb-1 font-inter text-[10px] font-semibold uppercase tracking-wider text-smile-description">Details</p>
...
export default function AdminAuditLogsPage() {
  ...
  const logs = !searchIsUserId && trimmedSearch
    ? rawLogs.filter((l) => (l.full_name ?? '').toLowerCase().includes(trimmedSearch.toLowerCase()))
    : rawLogs;
  const total = data?.meta?.total ?? 0;
  const totalPages = Math.max(1, Math.ceil(total / LIMIT));

  const hasActiveFilters = Boolean(searchInput || actionFilter || resourceFilter || fromDate || toDate);
  const resetFilters = () => {
    setSearchInput('');
    setActionFilter('');
    setResourceFilter('');
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-3 px-6 py-5">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-500 to-indigo-500 shadow-[0_4px_12px_rgba(99,102,241,0.35)]">
                <Icon icon="lucide:scroll-text" width={20} className="text-white" />
              </div>
              <div>
                <h1 className="font-poppins text-xl font-semibold text-smile-primary-dark">Access Audit Log</h1>
                <p className="font-inter text-xs text-smile-description">
                  {total > 0
                    ? <><span className="font-semibold text-indigo-600">{total}</span> events recorded</>
          ...
          <div className="relative space-y-3 p-5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <select
                ...
                style={actionFilter ? { background: '#6366F1', borderColor: '#6366F1', color: '#fff' } : inputStyle}
              >
                <option value="" style={{ background: '#fff', color: '#1a1a1a' }}>All actions</option>
                {AUDIT_ACTION_OPTIONS.map((opt) => (
                  <option key={opt.value} value={opt.value} style={{ background: '#fff', color: '#1a1a1a' }}>{opt.label}</option>
                ))}
              </select>
              ...
              <select
                ...
                className="cursor-pointer appearance-none rounded-xl border py-2.5 pl-3 pr-8 font-inter text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-indigo-400/30"
                style={resourceFilter ? { background: '#6366F1', borderColor: '#6366F1', color: '#fff' } : inputStyle}
              >
                <option value="" style={{ background: '#fff', color: '#1a1a1a' }}>All resources</option>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center gap-3 py-24">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-500/10">
                <Icon icon="line-md:loading-twotone-loop" width={28} className="text-indigo-500" />
              </div>
              <p className="font-inter text-sm text-smile-description">Loading audit log…</p>
            </div>
          ) : isError ? (
            <div className="flex flex-col items-center justify-center gap-3 py-24">
              ...
              <p className="font-inter text-sm font-medium text-smile-title">Failed to load audit log</p>
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-24">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-500/5">
                <Icon icon="lucide:inbox" width={24} className="text-smile-description/40" />
              </div>
              <p className="font-inter text-sm text-smile-description">
                {hasActiveFilters ? 'No events match these filters' : 'No events recorded yet'}
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="relative hidden overflow-x-auto sm:block">
                <table className="w-full min-w-[760px]">
                  <thead>
                    <tr className="border-b" style={{ borderColor: 'var(--surface-panel-border)', background: 'rgba(99,102,241,0.05)' }}>
                      <th className="w-10 px-3 py-3.5" />
                      <th className="whitespace-nowrap px-4 py-3.5 text-left font-inter text-[10px] font-bold uppercase tracking-[2px] text-indigo-600/70">Actor</th>
                      <th className="whitespace-nowrap px-4 py-3.5 text-left font-inter text-[10px] font-bold uppercase tracking-[2px] text-indigo-600/70">Action</th>
                      <th className="whitespace-nowrap px-4 py-3.5 text-left font-inter text-[10px] font-bold uppercase tracking-[2px] text-indigo-600/70">Resource</th>
                      <th className="whitespace-nowrap px-4 py-3.5 text-left font-inter text-[10px] font-bold uppercase tracking-[2px] text-indigo-600/70">IP Address</th>
                      <th className="whitespace-nowrap px-4 py-3.5 text-left font-inter text-[10px] font-bold uppercase tracking-[2px] text-indigo-600/70">Time</th>
                    </tr>
                  </thead>
                  <tbody>
                    {logs.map((log, i) => {
                            ...
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-2.5">
                                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-100 font-inter text-xs font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-300">
                                  {log.full_name ? log.full_name.charAt(0).toUpperCase() : <Icon icon="lucide:cog" width={13} />}
                                </div>
                                <div className="min-w-0">
                                  <p className="truncate font-inter text-sm font-semibold text-smile-primary-dark">{log.full_name ?? 'System'}</p>
                                  {log.user_id && <p className="truncate font-mono text-[10px] text-smile-description">{log.user_id}</p>}
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3.5">
                              <span className={`inline-flex items-center rounded-full px-2.5 py-1 font-inter text-xs font-semibold ${actionMeta.className}`}>
                                {actionMeta.label}
                              </span>
                            </td>
                            <td className="px-4 py-3.5">
                              <span className="font-inter text-sm text-smile-description">{getAuditResourceLabel(log.resource)}</span>
                            </td>
                            <td className="px-4 py-3.5">
                              <span className="font-mono text-xs text-smile-description">{log.ip_address ?? '—'}</span>
                            </td>
                            <td className="px-4 py-3.5">
                              <span className="whitespace-nowrap font-inter text-xs text-smile-description" title={formatDateTime(log.created_at)}>
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              <tr className="border-b" style={{ borderColor: 'var(--surface-panel-border)' }}>
                                <td colSpan={6} className="p-0" style={{ background: 'var(--surface-input-bg)' }}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col items-center pt-1.5">
                        <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${getAuditActionDotColor(log.action)}`} />
                        {i < logs.length - 1 && <span className="mt-1 w-px flex-1" style={{ background: 'var(--surface-panel-border)' }} />}
                      </div>
                      <div className="min-w-0 flex-1 space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 font-inter text-[11px] font-semibold ${actionMeta.className}`}>
                            {actionMeta.label}
                          </span>
                          <span className="shrink-0 font-inter text-[11px] text-smile-description">{formatRelativeTime(log.created_at)}</span>
                        </div>
                        <p className="truncate font-inter text-sm font-medium text-smile-title">{log.full_name ?? 'System'}</p>
                        <p className="font-inter text-[11px] text-smile-description">
                          {getAuditResourceLabel(log.resource)}{log.ip_address ? ` · ${log.ip_address}` : ''}
                        </p>
                        <button
                          type="button"
                          onClick={() => toggleExpand(log.log_id)}
          ...
          {!isLoading && !isError && logs.length > 0 && (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="rounded-lg bg-indigo-500/10 px-2.5 py-1 font-inter text-xs font-semibold text-indigo-600">{total}</span>
                <span className="font-inter text-xs text-smile-description">events</span>

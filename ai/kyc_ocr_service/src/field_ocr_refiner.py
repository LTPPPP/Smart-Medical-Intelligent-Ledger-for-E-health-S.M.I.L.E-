...
TARGET_FIELDS = ("full_name", "place_of_origin", "place_of_residence")
...
class FieldCropOcrRefiner:
    ...
    def refine(
        ...
        *,
        ...
        for crop in crops:
            if crop.name == "full_name" and refined.full_name:
                continue
            if (
    ...
    def _recognize_crop_fields(self, crop: FieldCrop) -> CccdFields:
        ...
        for variant_path in _build_crop_variants(crop):
            lines = self.ocr_engine.recognize(variant_path)
            crop_fields = _fields_from_crop_lines(lines)
            best = _merge_refined_fields(best, crop_fields)
            if crop.name != "address_block" and _crop_satisfied(crop.name, best):
                break
        return best

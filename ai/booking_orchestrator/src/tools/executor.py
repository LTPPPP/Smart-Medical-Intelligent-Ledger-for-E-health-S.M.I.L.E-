...
class ClinicalEmrToolClient:
    def __init__(self, base_url: str, internal_token: str | None = None):
...
class ToolExecutor:
    ...
    def _execute_clinical_tool(self, tool_name: str, data: Any) -> dict[str, Any]:
        ...
        if tool_name == "get_available_slots":
            return self.clinical_client.request("GET", "/v1/slots", params=payload)
        if tool_name == "hold_slot":
            slot_id = payload.pop("slot_id")
            return self.clinical_client.request(
                "POST", f"/v1/slots/{slot_id}/hold", json=payload
            )
        if tool_name == "confirm_booking":
            return self.clinical_client.request("POST", "/v1/bookings/confirm", json=payload)
        if tool_name == "release_hold":
            return self.clinical_client.request("POST", "/v1/holds/release", json=payload)
        ...
        if tool_name == "add_to_waitlist":
            return self.clinical_client.request("POST", "/v1/waitlist", json=payload)
        if tool_name == "check_waitlist_matches":
            return self.clinical_client.request(
                "GET", f"/v1/waitlist/matches/{payload['slot_id']}"
            )
        if tool_name == "send_email_notification":
            return self.clinical_client.request(
                "POST", "/v1/agent/email-notifications", json=payload
            )
        if tool_name == "create_handoff_ticket":
            return self.clinical_client.request(
                "POST", "/v1/agent/handoff-tickets", json=payload
            )

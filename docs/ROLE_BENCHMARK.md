...
## 0.5 Ma trận trang/tính năng theo role (S.M.I.L.E — hiện trạng & mục tiêu)

> Bảng tham chiếu nhanh — chi tiết nghiệp vụ và gap bảo mật xem `main_flow.md` **Phần J**.
> Nguồn routes: `frontend/web/src/app/` (Next.js App Router), `shared/constants/nav.ts`, `middleware.ts`.

**Ký hiệu:** ✅ Được phép | ⚠️ Giới hạn/điều kiện | 🚫 Không được phép | ⏸️ Phase sau

| Nhóm | Trang / Route | Patient | Receptionist | Nurse | Doctor | Admin |
|---|---|:---:|:---:|:---:|:---:|:---:|
| **Public / Auth** | `/`, `/login`, `/register` | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Dashboard** | `/dashboard` | ✅ Patient view | ✅ Front-desk | ✅ Nurse | ✅ Worklist | ✅ Admin |
| **Hồ sơ / Chat** | `/profile`, `/chat` | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Lịch hẹn** | `/appointments` (danh sách) | 🚫 | ✅ toàn clinic | ✅ | ✅ ca mình | ✅ |
| **Lịch hẹn** | `/appointments/new` | ✅ | ✅ | 🚫 | 🚫 | ✅ |
| **Lịch hẹn** | `/appointments/[id]` (chi tiết) | ✅ ca mình | ✅ | ✅ | ✅ ca mình | ✅ |
| **Lịch hẹn** | `/appointments/[id]/edit` | 🚫 | ✅ | 🚫 | 🚫 | ✅ |
| **Lịch hẹn** | `/appointments/[id]/payment` | ✅ | ✅ | 🚫 | 🚫 | ✅ |
| **Bệnh nhân** | `/patients` (danh sách) | 🚫 | ✅ | ⚠️ | ✅ ca được giao | ✅ |
| **Bệnh nhân** | `/patients/[id]` (hành chính) | 🚫 | ✅ | ⚠️ | ✅ ca được giao | ✅ |
...
### Gap bảo mật cần fix (tóm tắt)

| Gap | Hiện trạng | Action cần làm |
|---|---|---|
| FE middleware bypass | `DISABLE_AUTH_GUARD = true` tại `middleware.ts:15` | Re-enable |
| Thiếu role check trên page | Chỉ `/admin/*` và payment page có `<ProtectedRoute>` | Thêm `requiredRoles` cho từng page |
| Gateway không bắt medical routes | `/patients`, `/medical-records`, `/examination-sessions`, `/dental-images` không yêu cầu token | Extend `requiresTrustedIdentity()` |
| BE `@Roles` chưa enforce | `RolesGuard` chưa đăng ký trong `clinical-emr-service` | Đăng ký guard toàn cục hoặc per-controller |

> Chi tiết đầy đủ (bằng chứng code, line numbers, acceptance criteria) → xem `main_flow.md` **Phần J** và **F3.1**.
